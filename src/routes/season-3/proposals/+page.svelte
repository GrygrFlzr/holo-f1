...
import type { PageProps } from './$types';
    ...
    let ayaviProposal = data.teams.map(team => {
        let grandsPrix = data.grandsPrix.slice(0,4).map((gpName, gpIndex) => {
            ...
            const records = data.scores.filter(
                record => record.round === gpNum && record.team === team.name
            )
    ...
    let dakkyunProposal = data.teams.map(team => {
        let grandsPrix = data.grandsPrix.slice(0,4).map((gpName, gpIndex) => {
            const gpNum = gpIndex + 1;
            const records = data.scores.filter(
                record => record.round === gpNum && record.team === team.name
            ...
            return {
                ...
                name: gpName,
                records: records,
                score: records.length === 0 ? 0 : records.map(rec => rec.score).reduce((a, b) => a + b, 0) / records.length,
            };
        });
        return {
            name: team.name,
